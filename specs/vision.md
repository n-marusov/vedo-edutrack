...
| **1** | **F1: Движок маршрутов** | Вычисление персонального маршрута: `Маршрут = f(позиция ученика, цель, педагогическая концепция, онтология) → маршрут`. Автоматический пересчёт при триггерах: прогресс, обновление онтологии, смена цели. Учёт типов связей (`hasStrictPrerequisite` / `hasSoftPrerequisite` / `enriches`). Подбор ресурсов под стиль ученика. | Нелинейные, всегда актуальные маршруты. Межпредметные открытия. Экономия времени методиста: не строить маршрут вручную. |
...
| F1.5 | Анализ разрыва до цели | «Что нужно освоить для роли X, чего ещё нет» — неосвоенные модули на пути к цели (не путать с дефицитами покрытия формальной рамки, F2.8). Вход: текущая позиция + целевая роль. Выход: список необходимых модулей, критический путь, оценка времени. |
...
| F4.6 | Карта лакун (диагностический вид) | Отдельный вид карты знаний: только корневые лакуны (первый неосвоенный пререквизит в цепочке strict-связей) и их каскадные связи. Родитель видит корень проблемы одним взглядом: красные узлы — лакуны, стрелки — какие предметы они блокируют, зелёные узлы рядом — «закрой это, и разблокируются эти три темы». Заменяет ручную сверку в Excel и догадки родителя о причине трудностей. |
...
| Код уровня 3 | Функция | Описание |
|---|---|---|
| F2.8.1 | Расчёт покрытия | `coverage = требования рамки, закрытые освоенными модулями / всего требований для контекста`. Модуль закрывает требование, если привязан к нему (F2.8.5). |
...
| F2.8.4 | Минимальный закрывающий набор | Набор модулей, которые нужно освоить до контрольной точки для достижения целевого покрытия (не путать с критическим путём — цепочкой минимального срока, F1.3). |
...
**Фаза 1 — MVP (2 кв.):**
- **M1. Базовая инфраструктура:** порт онтологии (F0), движок маршрутов с strict-связями и диагностикой лакун (F1 без F1.8), план обучения с бинарным прогнозом (F2.1–F2.3), каталог ресурсов (F3.1), каркас REST API (F6.1). Стартовая онтология: 1000+ тем 5–11 классов, 500+ связей, ФГОС-привязка, 3–5 педконцепций.
- **M2. Семейное образование — «Дай пять»:** карта знаний (F4.1), карта лакун (F4.6), дашборды ученика/родителя (F4.2, F4.4), панель управления группой (F4.7), конструктор маршрутов (F4.3), покрытие ФГОС и аттестационная готовность (F2.8–F2.9), 50+ историй и 30+ проектных идей (F5), демо-редактор.
- **M3. Корпоративный онбординг — «Вектор Компетенций»:** онбординг-план (навыки + домен + ценности + сервисы + социализация), метрика time-to-productivity, базовый корпоративный дашборд. Пилот: 1-2 компании.
- **M4. Интеграционный слой:** продуктовый REST API, read-only SPARQL-эндпоинт, вебхуки (`module.mastered`, `plan.deviated`), MCP-сервер.

**Фаза 2 — Обогащение и B2B (3 кв.):**
- **M5. Обогащение онтологии и маршрутов:** все 5 типов связей, маршруты с учётом педконцепций (F1.8), каскадный пересчёт при обновлении онтологии (F0.3), подбор ресурсов под стиль обучения, сложный трехуровневый прогноз (±10%), 200+ историй и 100+ проектных идей.
