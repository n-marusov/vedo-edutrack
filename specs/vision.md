...
**VEDO EduTrack** — сервис (движок) образовательных маршрутов на базе VEDO Hub: проектирование и исполнение индивидуальных маршрутов, расчёт планов и покрытия формальных рамок. Название едино для ядра, API, документации и интеграций; приложения, использующие сервис, носят собственные имена (reference-приложения: «Дай пять», «Свой Вектор», «Вектор Компетенций»).
...
- **[G5] Сетевой:** Публичная онтология растёт: 1000+ тем на старте, 5000+ через 3 года. Каждый вклад методиста увеличивает ценность маршрутов для всех пользователей (онтология и вклад методистов — зона VEDO Hub; сервис потребляет обновления через API, F6).
...
Единый сервис обслуживает приложения из разных отраслей: механика «маршрут + расписание + план + покрытие формальной рамки» универсальна, а отрасль определяет свою формальную рамку (стандарт) и онтологию. Каждое применение — отдельный доменный модуль или reference-приложение.
...
4. **Доменные модули** (готовые пакеты-вертикали): «Медицина» (онтология + НМО + шаблоны), «Финансы» (онтология + 115-ФЗ + шаблоны), «Языки» (онтология + CEFR + шаблоны).
5. **Фронтенд** (свой код): интегратор пишет свой UI, но может заимствовать готовые компоненты из демо (карта знаний, дашборды, конструктор) как основу своего фронтенда.
...
|---|---|---|---|
| **1** | **F1: Движок маршрутов** | Вычисление персонального маршрута: `Маршрут = f(ученик, онтология) → маршрут`. Автоматический пересчёт при триггерах: прогресс, обновление онтологии, смена цели. Учёт типов связей (`hasStrictPrerequisite` / `hasSoftPrerequisite` / `enriches`). Подбор ресурсов под стиль ученика. | Нелинейные, всегда актуальные маршруты. Межпредметные открытия. Экономия времени методиста: не строить маршрут вручную. |
| **2** | **F2: План обучения** | Фиксация снэпшота маршрута на контрольных точках (начало четверти, семестра, онбординга). Отображение «план и факт»: плановые даты, реальные даты, отклонения, причины. Прогноз выполнения к контрольной точке. Генерация плана под входные ограничения — контрольные точки и ФГОС/профстандарт (F2.7), покрытие и дефициты (F2.8), аттестационная готовность (F2.9). | Подотчётность и прозрачность: родитель/HR видят «что запланировано и как идём» **и соответствие плана ФГОС/формальной рамке** — покрытие требований, дефициты, прогноз к аттестации. Спокойствие перед контрольной точкой: видно, успевает или под риском. |
| **4** | **F3: Ресурсы** | Привязка всех ресурсов к модулям: учебные материалы (видео/текст/интерактив/учебники), репетиторы, лабораторное оборудование, доступы (музеи), деньги. Фильтры по формату, стилю, бюджету. Доступность и обеспеченность, затраты и бюджет маршрута. | **Планирование бюджета и предсказуемое обеспечение:** все ресурсы видны заранее — что нужно, сколько стоит, доступно ли; никаких неожиданных трат и «когда припрёт». |
| **5** | **F4: Визуализация графа знаний и маршрута** | Карта знаний: граф тем и связей с подсветкой прогресса. Дашборд ученика: «доступно сейчас», «критический путь», «план и факт». Конструктор маршрутов: выбор цели → визуализация маршрута. Фронтенд выполняет две функции: (1) общее демо возможностей сервиса; (2) сервис для семейного образования (reference-приложение «Дай пять»). Демо служит библиотекой переиспользуемых компонентов: интегратор пишет свой фронтенд, но может заимствовать готовые компоненты (карта знаний, дашборды, конструктор). | Понимание структуры знаний с первого взгляда. Прозрачность для родителя и HR. Демо показывает возможности сервиса и даёт интегратору готовые компоненты для своего фронтенда; сервис «Дай пять» — рабочий продукт для семейного образования. |
| **6** | **F5: Связь с практикой и жизнью** | Подбирает истории, контекст и проектные идеи к шагам маршрута (по связям графа `appliesTo`/`enriches`) и подаёт их в момент освоения модуля — чтобы ученик видел «зачем это знать» и связь с реальной жизнью. Практическая мотивация даже для фундаментальных знаний: логарифмы → музыка и сейсмология, теорема Пифагора → строительство, химия → медицина. | Вовлечённость и мотивация: ученик видит смысл изучаемого (включая абстрактные темы), меньше «скучно, зачем мне это» (P21). |
| **7** | **F6: Интеграции** | REST API, SPARQL-эндпоинт, SDK-клиенты (TypeScript/Python/Java), MCP-сервер, коннекторы к LMS (WebTutor, iSpring), Webhooks, SSO/SAML. Контент-агностичность: контент платформ подключается как ресурсы модулей. Предоставляется как **VEDO EduTrack API**. | Сервис становится инфраструктурным слоем: EdTech подключается за 1 день вместо 4 месяцев построения собственного графа, оставаясь владельцем контента и аудитории. |

### 2.2. Декомпозиция функций на 3 уровня

#### Уровень 1 — Ключевые бизнес-функции (6 доменов)

| ID | Функциональный домен | Соответствие бизнес-цели | Описание домена |
|---|---|---|---|
| **F1** | Движок маршрутов | G1, G3, G6 | Вычисление маршрута по графу. Учёт strict/soft/enrich-связей. Пересчёт по триггерам: прогресс, обновление онтологии, смена цели. Подбор ресурсов |
| **F2** | План обучения | G1, G4 | Фиксация снэпшота маршрута на период. План и факт. Прогноз выполнения. Пересчёт при смене периода/цели |
...
| **F6** | Интеграции | G2, G6 | REST API, SDK-клиенты, MCP-сервер, коннекторы к LMS, Webhooks, SSO/SAML. Контент-агностичность |
...
| F1.5 | Анализ дефицитов до цели | «Что нужно освоить для роли X, чего ещё нет». Вход: текущая позиция + целевая роль. Выход: список необходимых модулей, критический путь, оценка времени. |
...
| F1.8 | Маршрут с учётом педагогической концепции | Фильтрация и группировка модулей в соответствии с выбранной концепцией. Примеры: концепция «Спиральное обучение» — тема «Функции» появляется в 7 классе (линейные), 8 классе (квадратичные), 9 классе (показательные) с возрастающей глубиной; маршрут строит витки, а не линейную цепочку. Концепция «Проектное погружение» — модули из разных предметов группируются вокруг проекта «Экосистема»: биология (виды и популяции) + математика (проценты для расчёта биомассы) + химия (круговорот веществ) + география (климатические зоны). Концепция «От практики к теории» — эксперимент → наблюдение → закономерность → формула, а не наоборот. Родитель / методист выбирает концепцию при построении маршрута; концептуальная онтология пополняется сообществом. |
| F1.9 | Проверочные задания и калибровка | Генерирует проверочные задания под конкретный модуль и уровень освоения; калибрует задания по трудности (модели IRT/Раша), контролирует валидность и надёжность. Результат — оценка освоения с погрешностью, вход для позиции, прогноза и диагностики лакун. |
| F1.10 | Требования рамки как фильтр | При вычислении маршрута: если контекст = «школьное образование», модули, закрывающие ФГОС-дефициты, получают повышенный приоритет за N недель до аттестации. |
| F1.11 | Сверка «живого» знания с формальной рамкой | Для каждого модуля — маппинг на конкретные формулировки ФГОС. Ученик знает тему → система показывает: «Вот что именно спросят на аттестации по этой теме.» |

##### F2. План обучения

| Код | Функция | Описание |
|---|---|---|
| F2.1 | Фиксация плана | План = маршрут (последовательность модулей) + временные метки (плановые даты начала и завершения для каждого модуля). План фиксируется на контрольной точке. После фиксации маршрут продолжает пересчитываться независимо. Система всегда показывает два слоя: «что рекомендуем сейчас» (маршрут) и «что запланировали» (план). |
| F2.2 | План и факт | Для каждого модуля плана: плановая дата vs реальная дата завершения, отклонение в днях, причина отклонения (ускорение / требовалось больше практики / перерыв). |
| F2.3 | Прогноз выполнения | На основе текущего темпа и оставшихся модулей — прогноз: «успевает / под риском (отклонение в пределах допуска, < 15%) / не успевает» к контрольной точке. |
| F2.4 | Пересчёт плана | Пересчёт плана не автоматический. Условия: новый период, смена цели, провал контрольной точки, или ручная инициация. Если новый маршрут содержит существенные изменения (>15% модулей или >2 недель экономии по сравнению с планом), система показывает дельту «план vs актуальный маршрут» и предлагает пересмотр. Решение принимает пользователь. Пока новый план не зафиксирован, прогресс измеряется от старого плана, рекомендации — от нового маршрута. |
| F2.5 | Уведомления об отклонениях | При отставании > N дней от плана — уведомление родителю / HR / ученику: «Отставание на 5 дней по модулю "Производные". Рекомендация: дополнительные 2 часа на этой неделе.» |
| F2.6 | Межпредметная синхронизация расписания | Вычисляет расписание с учётом межпредметных связей: модули разных предметов выстраиваются в правильном порядке во времени (проект «Экосистема» → проценты по математике раньше биологии). Групповые занятия ставятся в точки схождения маршрутов группы, чтобы каждый ученик приходил подготовленным. |
| F2.7 | Контрольные точки и формальные рамки (входные ограничения) | Входные ограничения для генерации плана: контрольные точки (даты аттестации, конец четверти) и формальные рамки (ФГОС, профстандарт) из онтологии. План генерируется под эти ограничения: целевое покрытие к дате. |
| F2.8 | Покрытие и дефициты (проекция) | Доля закрытых требований рамки: `coverage = освоенные модули с привязкой / всего требований`. Дефициты — незакрытые требования с приоритетом (жёсткий пререквизит > ядро > опциональное). |
| F2.9 | Аттестационная готовность (проекция) | Сводный отчёт перед контрольной точкой: покрытие по доменам, дефициты, критический путь закрытия, прогноз времени. |

##### F3. Ресурсы

| Код | Функция | Описание |
|---|---|---|
| F3.1 | Привязка ресурсов к модулям | Все ресурсы, необходимые для прохождения шага: учебные материалы (видео, статья, интерактив, учебник, книга), репетиторы/наставники, лабораторное оборудование, доступы (музеи, лаборатории), деньги (стоимость). С указанием типа, формата, стиля, стоимости, источника. |
...
| Код | Функция | Описание |
|---|---|---|
| F6.1 | Интеграция с VEDO Hub (источник онтологии) | Подключение к VEDO Hub через REST API/MCP как к единому источнику онтологии. Польза для конечного пользователя: маршруты всегда строятся на самой полной и актуальной карте знаний; обновления от сообщества методистов доходят автоматически, без ручной настройки. |
| F6.2 | REST API маршрутов | Эндпоинты: вычисление маршрута, получение прогресса, план/факт, покрытие ФГОС. Для EdTech-платформ. |
| F6.3 | SPARQL endpoint | Прямые запросы к графу знаний: «доступные модули», «кратчайший путь», «дефициты», «ресурсы по теме». Read-only, с аутентификацией и rate limiting. |
| F6.4 | Интеграция с LMS | Готовые коннекторы: WebTutor, iSpring, SAP SuccessFactors. Двусторонний обмен: маршрут передаётся в LMS как учебный план, прогресс и оценки возвращаются из LMS. |
| F6.5 | Webhooks | Уведомления: `route.recalculated`, `module.mastered`, `plan.deviated`, `fgos.risk_detected`. Идемпотентные — дублирование событий не ломает состояние. |
| F6.6 | Enterprise SSO/SAML | Keycloak-интеграция для корпоративных клиентов. |
| F6.7 | SDK-клиенты | Готовые библиотеки (TypeScript, Python, Java) для встраивания движка маршрутов в платформу: вычисление маршрута, приём событий, получение проекций. «Подключение за 1 день». |
| F6.8 | MCP-сервер | Стандартизированный интерфейс для AI-агентов и внешних сервисов: семантический поиск по онтологии, навигация по графу, извлечение контекста модуля. |
| F6.9 | Контент-агностичность | Контент платформы (видео, курсы, тренажёры) подключается как ресурсы модулей — EduTrack не хостит контент, а связывает его в маршруты. Платформа остаётся владельцем контента и аудитории. |

---

#### Уровень 3 — Детализация (примеры)

##### F1. Движок маршрутов → F1.1 Вычисление маршрута
...
| F1.1.3 | Вес связей | strict = вес 0 (обязательно включить в путь), soft = вес 1 (включить если сокращает путь), enrich = вес 2 (включить только если есть время/интерес). |
...
##### F2. План обучения → F2.8 Покрытие ФГОС

| Код уровня 3 | Функция | Описание |
|---|---|---|
| F2.8.1 | Расчёт покрытия | `coverage = освоенные модули с ФГОС-привязкой / всего ФГОС-требований для класса`. |
| F2.8.2 | Прогноз покрытия | `projected = (освоенные + в процессе × factor) / всего`. Factor зависит от % завершения и темпа. |
| F2.8.3 | Уровень риска | `risk = f(coverage, projected, days_until_deadline)`. Зелёный: успевает. Жёлтый: под риском (отклонение вышло за допуск, > 15%). Красный: не успевает. |
| F2.8.4 | Критический путь до аттестации | Минимальный набор модулей, которые нужно освоить до контрольной точки для достижения целевого покрытия. |
| F2.8.5 | Сравнение «живого» и формального | Для каждого освоенного модуля — маппинг на формулировку ФГОС. «Тема освоена: ✅. Формулировка для аттестации: [текст из ФГОС]. Пробный вопрос: [пример].» |
...
flowchart LR
    ...
    Root --> F1["F1: Движок маршрутов"]
    Root --> F2["F2: План обучения"]
    Root --> F3["F3: Ресурсы"]
    ...
    F1 --> F1_5["F1.5 Анализ дефицитов"]
    F1 --> F1_6["F1.6 Каскад пересчёта"]
    F1 --> F1_9["F1.9 Проверочные задания"]
    F1 --> F1_10["F1.10 Рамка как фильтр"]
    F1 --> F1_11["F1.11 Сверка с рамкой"]
    ...
    %% F2: План обучения
    F2 --> F2_1["F2.1 Фиксация плана"]
    F2 --> F2_2["F2.2 План и факт"]
    F2 --> F2_3["F2.3 Прогноз выполнения"]
    F2 --> F2_4["F2.4 Пересчёт плана"]
    F2 --> F2_5["F2.5 Уведомления"]
    F2 --> F2_6["F2.6 Синхронизация расписания"]
    F2 --> F2_7["F2.7 Входные ограничения"]
    F2 --> F2_8["F2.8 Покрытие и дефициты"]
    F2 --> F2_9["F2.9 Аттестационная готовность"]
    ...
    %% F6: Интеграции
    F6 --> F6_1["F6.1 VEDO Hub (источник)"]
    F6 --> F6_2["F6.2 REST API"]
    F6 --> F6_3["F6.3 SPARQL endpoint"]
    F6 --> F6_4["F6.4 Интеграция с LMS"]
    F6 --> F6_5["F6.5 Webhooks"]
    F6 --> F6_6["F6.6 Enterprise SSO/SAML"]
    F6 --> F6_7["F6.7 SDK-клиенты"]
    F6 --> F6_8["F6.8 MCP-сервер"]
    F6 --> F6_9["F6.9 Контент-агностичность"]
```
...
| `OnboardingCompleted` | Все essential-модули роли освоены | Вертикаль «Вектор Компетенций» | HR-дашборд | Создать отчёт: фактическое время онбординга, отклонения от плана, возврат инвестиций (сэкономлено vs ручной онбординг). |
...
- **Трек А — B2C-воронка («Дай пять»):** родители на семейном обучении. Карта знаний (F4.1) + диагностика лакун (F1.7). Онтология: математика 5-7 классов. Гипотеза: «Родитель заплатит за спокойствие перед аттестацией». Метрика: NPS ≥ 40.
- **Трек Б — B2B-якорь («Вектор Компетенций»):** 1-2 пилотные компании. Онбординг-план (вертикаль «Вектор Компетенций»). Гипотеза: «Сокращение срока выхода на продуктивность». Метрика: 2 недели вместо 4.
...
**F2. План обучения (MVP) — дополнение:**
...
- Корпоративные сценарии (онбординг, карьерные треки, соблюдение требований) — не функции сервиса, а вертикаль «Вектор Компетенций» (reference-приложение), развивается отдельно.
...
- Маршрут с учётом педагогической концепции (F1.8): движок строит маршрут в зависимости от выбранной концепции — спиральные витки, проектное группирование модулей, смена порядка «от практики к теории».
- 200+ историй и 100+ проектных идей (F5).
- Интеграция с LMS «из коробки»: WebTutor, iSpring.

**Этап 3 — Вертикали, соблюдение требований и вклад в онтологию:**
- Корпоративная вертикаль «Вектор Компетенций»: онбординг, карьерные треки, обучение соблюдению требований (привязка регуляторных требований к модулям и рабочим сценариям) — на универсальных механиках сервиса.
- Корпоративный дашборд вертикали: онбординг-воронка, срок выхода на продуктивность, покрытие регуляторных требований.
- Предложение вклада в онтологию через API Hub (создание тем/связей по мере роста сообщества).
