...
**VEDO EduTrack** — фреймворк для создания приложений, проектирующих и исполняющих образовательные маршруты. Название едино для ядра, API, документации и интеграций; приложения, построенные на фреймворке, носят собственные имена (reference-приложения: «Дай пять», «Свой Вектор», «Вектор Компетенций»).
...
- **Для EdTech-платформы:** Форкнуть публичную онтологию знаний за 1 день — вместо 4 месяцев ручного построения графа. Добавить свои курсы как приватную надстройку. Получать обновления онтологии от сообщества методистов бесплатно. Использовать API маршрутов как движок: SPARQL-запрос → персональный маршрут → рекомендации с пониманием межпредметных связей. Встроить движок через SDK-клиенты (TypeScript/Python/Java) и получать события через вебхуки. Платформа остаётся владельцем контента и аудитории — EduTrack лишь связывает её курсы в маршруты. (см. G2, G5)
...
| **G5** | Запустить сетевой эффект публичной онтологии знаний — каждый вклад методиста обогащает маршруты тысяч учащихся | **S**: Методисты и EdTech-платформы пополняют публичную онтологию через форки и запросы на слияние. Одна связь, добавленная методистом из Новосибирска, обогащает маршруты учеников в Москве, Казани и Владивостоке.<br>**M**: 5000+ тем, 8000+ связей через 3 года. 200+ активных контрибьюторов. Каждый вклад достигает тысяч учащихся через форки платформ.<br>**A**: Форк-модель VEDO Hub + социальный хаб (профили, рейтинг, DOI) + LLM-извлечение из ФГОС/учебников.<br>**R**: P12, P13 — методисты изолированы, стандарты дублируются, а каждый вклад должен работать на тысячи учащихся, а не на один Excel-файл.<br>**T**: К концу 4 кв. | 5000+ тем, 8000+ связей, 200+ контрибьюторов. Связь из Новосибирска — в маршрутах тысяч учеников по всей стране. | 4 кв. | P12, P13 | F1, F7 |
...
- **[G5] Сетевой:** Публичная онтология растёт: 1000+ тем на старте, 5000+ через 3 года. Каждый вклад методиста увеличивает ценность маршрутов для всех пользователей.
...
> **VEDO EduTrack — фреймворк образовательных маршрутов на базе VEDO Hub: не «платформа с курсами», а маршрут к цели, который живёт и перестраивается под ученика.**
...
Единый фреймворк обслуживает приложения из разных отраслей: механика «маршрут + расписание + план + покрытие формальной рамки» универсальна, а отрасль определяет свою формальную рамку (стандарт) и онтологию. Каждое применение — отдельный доменный модуль или reference-приложение.
...
**Четыре вертикали отраслевых применений:**
...
1. **Генерация маршрутов.** Вычисление персонального маршрута: `Маршрут = f(ученик, цель, педагогическая концепция, онтология)`. Алгоритм обходит граф через API Hub, учитывает типы связей, essential-ядро, выбранную концепцию и темп ученика.
...
5. **Подбор ресурсов и контекста.** Привязка материалов, историй и проектных идей к модулям маршрута с фильтром по формату, сложности и стилю обучения.
...
2. **Маршрут как функция, а не документ:** `Маршрут = f(ученик, цель, концепция, онтология) → маршрут`. Пересчитывается автоматически при любом триггере: освоен модуль → открылся веер возможностей; онтология обновилась → маршрут стал короче; цель изменилась → полная перестройка маршрута. Пересчёт существующего маршрута по актуальной позиции ученика, новой цели и действующим ограничениям (см. глоссарий, «Перестройка маршрута»). Маршрут — это представление, а не таблица.
...
| **2** | **F2: Движок маршрутов** | Вычисление персонального маршрута: `Маршрут = f(ученик, онтология) → маршрут`. Автоматический пересчёт при триггерах: прогресс, обновление онтологии, смена цели. Учёт типов связей (`hasStrictPrerequisite` / `hasSoftPrerequisite` / `enriches`). Подбор ресурсов под стиль ученика. | Нелинейные, всегда актуальные маршруты. Межпредметные открытия. Экономия времени методиста: не строить маршрут вручную. |
| **3** | **F3: План обучения** | Фиксация снэпшота маршрута на контрольных точках (начало четверти, семестра, онбординга). Отображение «план и факт»: плановые даты, реальные даты, отклонения, причины. Прогноз выполнения к контрольной точке. | Подотчётность и прозрачность. Родитель/HR видят не только «что доступно», но и «что запланировано и как идём». |
...
| **6** | **F6: Визуализация графа знаний и маршрута** | Карта знаний: граф тем и связей с подсветкой прогресса. Дашборд ученика: «доступно сейчас», «критический путь», «план и факт». Конструктор маршрутов: выбор цели → визуализация маршрута. | Понимание структуры знаний с первого взгляда. Прозрачность для родителя и HR. |
...
| **F2** | Движок маршрутов | G1, G3, G6 | Вычисление маршрута по графу. Учёт strict/soft/enrich-связей. Пересчёт по триггерам: прогресс, обновление онтологии, смена цели. Подбор ресурсов |
| **F3** | План обучения | G1, G4 | Фиксация снэпшота маршрута на период. План и факт. Прогноз выполнения. Пересчёт при смене периода/цели |
...
| **F6** | Визуализация | G1, G3 | Карта знаний, дашборд прогресса, конструктор маршрутов |
...
| **F9** | Интеграции | G2, G6 | REST API, SDK-клиенты, MCP-сервер, коннекторы к LMS, Webhooks, SSO/SAML. Контент-агностичность |
...
|---|---|---|
| F1.1 | Управление темами (KnowledgeModule) | Создание/редактирование тем. Поля: ID, Label, Description, Subject (математика, биология и др.), Difficulty (1-10), Level (базовый / углублённый / экспертный), EstimatedHours. Иерархия: тема → подтема. KnowledgeModule проектируется как минимальная проверяемая единица знания, для которой возможна однозначная бинарная оценка освоения. Декомпозиция темы на атомарные модули — обязанность методиста. Критерий освоения (`masteryCriteria`) является обязательным атрибутом модуля: тип оценки, порог, время, объём выборки. |
...
| Код | Функция | Описание |
|---|---|---|
| F2.1 | Вычисление маршрута | Двухэтапное вычисление: (1) копирование релевантного подграфа через API Hub — все модули, достижимые из текущей позиции ученика, с их связями и атрибутами; (2) алгоритм маршрутизации (кратчайший путь с весами) на скопированном подграфе, в памяти EduTrack, а не на стороне Hub. Вход: текущая позиция ученика (освоенные модули) и цель (роль / проект / экзамен / тема). Учёт типов связей: strict-связи обязательны для прохождения, soft — опциональны, enrich — бонус. Приоритет модуля — функция от статических весов связей и динамического контекста: близость дедлайна, essential-статус, темп ученика, выбранная концепция. Модули, закрывающие ФГОС-дефициты, повышают приоритет за N дней до аттестации (F4.4). Конфликт приоритетов разрешается в порядке: essential + strict → soft → enrich. |
| F2.2 | Триггеры перестройки маршрута | События, запускающие перестройку существующего маршрута: (1) Прогресс ученика: освоен модуль → пересчёт доступного. (2) Обновление онтологии: методист добавил тему/связь → каскадный пересчёт всех маршрутов. (3) Смена цели: «хочу в IT» → «хочу в геймдизайн» → полная перестройка. (4) Периодический: еженедельная проверка «что нового доступно». |
...
| F2.4 | Подбор ресурсов | Для каждого модуля маршрута — подбор материалов с фильтром по: стилю обучения (визуальный / текстовый / аудиальный), формату, сложности, длительности. |
...
| F2.6 | Каскад пересчёта | При изменении маршрута → пересчитывается план обучения (на новый период) → обновляется подборка ресурсов → обновляются рекомендации проектов и историй. |
...
| F3.3 | Прогноз выполнения | На основе текущего темпа и оставшихся модулей — прогноз: «успевает / под риском (отклонение в пределах допуска, < 15%) / не успевает» к контрольной точке. |
| F3.4 | Пересчёт плана | Пересчёт плана не автоматический. Условия: новый период, смена цели, провал контрольной точки, или ручная инициация. Если новый маршрут содержит существенные изменения (>15% модулей или >2 недель экономии по сравнению с планом), система показывает дельту «план vs актуальный маршрут» и предлагает пересмотр. Решение принимает пользователь. Пока новый план не зафиксирован, прогресс измеряется от старого плана, рекомендации — от нового маршрута. |
...
| F4.4 | Требования ФГОС как фильтр | При вычислении маршрута: если контекст = «школьное образование», модули, закрывающие ФГОС-дефициты, получают повышенный приоритет за N недель до аттестации. |
...
| F5.2 | Подбор под стиль ученика | При вычислении маршрута — фильтр ресурсов: «ученик предпочитает видео до 20 минут» → подбираются соответствующие материалы. |
...
| F6.3 | Конструктор маршрутов | Выбор цели (профессия / проект / экзамен) → визуализация маршрута → оценка времени → подтверждение → фиксация плана. |
...
| Код | Функция | Описание |
|---|---|---|
| F9.1 | REST API маршрутов | Эндпоинты: вычисление маршрута, получение прогресса, план/факт, покрытие ФГОС. Для EdTech-платформ. |
...
| F9.3 | Интеграция с LMS | Готовые коннекторы: WebTutor, iSpring, SAP SuccessFactors. Двусторонний обмен: маршрут передаётся в LMS как учебный план, прогресс и оценки возвращаются из LMS. |
| F9.4 | Webhooks | Уведомления: `route.recalculated`, `module.mastered`, `plan.deviated`, `fgos.risk_detected`. Идемпотентные — дублирование событий не ломает состояние. |
| F9.5 | Enterprise SSO/SAML | Keycloak-интеграция для корпоративных клиентов. |
| F9.6 | SDK-клиенты | Готовые библиотеки (TypeScript, Python, Java) для встраивания движка маршрутов в платформу: вычисление маршрута, приём событий, получение проекций. «Подключение за 1 день». |
| F9.7 | MCP-сервер | Стандартизированный интерфейс для AI-агентов и внешних сервисов: семантический поиск по онтологии, навигация по графу, извлечение контекста модуля. |
| F9.8 | Контент-агностичность | Контент платформы (видео, курсы, тренажёры) подключается как ресурсы модулей — EduTrack не хостит контент, а связывает его в маршруты. Платформа остаётся владельцем контента и аудитории. |

---

#### Уровень 3 — Детализация (примеры)

##### F2. Движок маршрутов → F2.1 Вычисление маршрута
...
| F2.1.5 | Временная оценка | Для каждого модуля — EstimatedHours. Суммарная оценка времени маршрута с буфером на темп ученика. |
...
| F4.1.3 | Уровень риска | `risk = f(coverage, projected, days_until_deadline)`. Зелёный: успевает. Жёлтый: под риском (отклонение вышло за допуск, > 15%). Красный: не успевает. |
...
flowchart LR
    ...
    F2 --> F2_2["F2.2 Триггеры перестройки"]
    ...
    F6 --> F6_3["F6.3 Конструктор маршрутов"]
    ...
    F9 --> F9_6["F9.6 SDK-клиенты"]
    F9 --> F9_7["F9.7 MCP-сервер"]
    F9 --> F9_8["F9.8 Контент-агностичность"]
```
...
| `ModuleMastered` | Прохождение теста / подтверждение преподавателем | Интерфейс / API (LMS) | VEDO Hub | Пометить модуль как освоенный (`hasMastered`), записать `masteredAt` и `masteryLevel`. Вызвать `RouteRecalculated`. |
...
- **Трек А — B2C-воронка («Дай пять»):** родители на семейном обучении. Карта знаний (F6.1) + диагностика лакун (F2.7). Онтология: математика 5-7 классов. Гипотеза: «Родитель заплатит за спокойствие перед аттестацией». Метрика: NPS ≥ 40.
- **Трек Б — B2B-якорь («Вектор Компетенций»):** 1-2 пилотные компании. Онбординг-план (F8.1). Гипотеза: «Сокращение срока выхода на продуктивность». Метрика: 2 недели вместо 4.

Общая инфраструктура (API, движок, план обучения) — сквозная. Остальной функционал MVP ниже распределён между треками.
...
- Фиксация снэпшота маршрута на начало периода.
...
**Что НЕ входит в MVP:**
- Маршрут с учётом педагогической концепции (F2.8) — MVP: линейный маршрут без привязки к концепциям. Этап 2: спиральные витки, проектное группирование, смена порядка «практика → теория».
...
**Этап 2 — Обогащение онтологии и маршрутов:**
- Все 5 типов межпредметных связей (добавить enriches, isAnalogousTo).
- Каскадный пересчёт маршрутов при обновлении онтологии: методист добавил связь → все форки получили обновление.
...
- Маршрут с учётом педагогической концепции (F2.8): движок строит маршрут в зависимости от выбранной концепции — спиральные витки, проектное группирование модулей, смена порядка «от практики к теории».
...
- On-premise развёртывание: VEDO Hub Enterprise + модуль «Маршруты» в корпоративном контуре.
...
| **Архитектура** | Вертикальный продукт | Фреймворк (одно ядро → приложения для многих отраслей) |
...
8. **Лёгкая интеграция** — SDK-клиенты (TypeScript, Python, Java), REST API, вебхуки, MCP-сервер; контент-агностичность: платформа остаётся владельцем контента и аудитории, EduTrack связывает его в маршруты. Подключение за 1 день вместо 4 месяцев. Одно ядро → приложения для многих отраслей.
...
- **Не «ещё одна платформа»** — фреймворк, на котором платформы строят свои приложения. Контент-платформы — партнёры, а не конкуренты: они производят курсы, EduTrack связывает их в маршруты.
...
|---|---|---|---|---|
| **Родитель (семейное обучение)** | Готовая карта знаний, привязка к ФГОС, спокойствие перед аттестацией. «Я не методист, но система подсказывает.» | Прямой пользователь (Community, «Дай пять») | F2, F3, F4, F6 | Не знает методики. Нужна максимальная простота и готовые маршруты. |
| **Ученик (семейное обучение)** | Нелинейные маршруты: интерес к динозаврам ведёт через биологию, географию, химию. Межпредметные открытия: «Пропорции — это ещё и музыка!» | Конечный пользователь | F2, F5, F6 | Теряет интерес при линейности. Нужна геймификация и контекст. |
...
| **Директор частной школы** | Индивидуальные маршруты для 80 учеников. Синхронизация проектов с ФГОС без ручной работы. Прозрачная отчётность родителям. | Прямой пользователь (Pro, «Свой Вектор») | F3, F4, F6 | Ограниченный бюджет. Важна автоматизация, а не ручное управление. |
| **CTO / CPO EdTech-платформы** | Готовый граф знаний за 1 день вместо 4 месяцев разработки. API маршрутов как движок. Обновления онтологии от сообщества бесплатно. | Пользователь API / Интегратор (Pro) | F1, F2, F7, F9 | Высокие лимиты API, SLA, документация. Беспокойство о зависимости от внешней онтологии. |
