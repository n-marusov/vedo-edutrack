...
| F6.4 | Webhooks | Уведомления: `route.recalculated`, `module.mastered`, `plan.deviated`, `fgos.risk_detected`. Идемпотентные — дублирование событий не ломает состояние. |
| F6.5 | Enterprise SSO/SAML | Keycloak-интеграция для корпоративных клиентов. |
| F6.6 | SDK-клиенты | Готовые библиотеки (TypeScript, Python, Java) для встраивания движка маршрутов в платформу: вычисление маршрута, приём событий, получение проекций. «Подключение за 1 день». |
| F6.7 | MCP-сервер | Стандартизированный интерфейс для AI-агентов и внешних сервисов: семантический поиск по онтологии, навигация по графу, извлечение контекста модуля. |
| F6.8 | Контент-агностичность | Контент платформы (видео, курсы, тренажёры) подключается как ресурсы модулей — EduTrack не хостит контент, а связывает его в маршруты. Платформа остаётся владельцем контента и аудитории. |
...
flowchart LR
    ...
    F6 --> F6_6["F6.6 SDK-клиенты"]
    F6 --> F6_7["F6.7 MCP-сервер"]
    F6 --> F6_8["F6.8 Контент-агностичность"]
```
...
- Базовые обновления онтологии (F0.3) — пересчёт маршрутов при изменении.
...
- Порт онтологии (F0): полная подписка на обновления, каскадный пересчёт всех маршрутов при изменении онтологии.
