...
> **Статус:** стек разработки — TBD. Основные ADR (выбор языка, фреймворка, БД, ORM) будут созданы после принятия стекового решения. До этого момента ADR фиксируют архитектурные инварианты, не зависящие от стека.
...
- **Стек — TBD**: ADR уровня `STACK` и `IMPL` отложены до выбора языка/фреймворка. Инвариантные архитектурные решения (границы сервиса, модель данных, API-контракт) принимаются на уровне `DES` независимо от стека.
