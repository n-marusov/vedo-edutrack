...
> **Статус:** язык и фреймворки зафиксированы (`ADR-DES.STACK.*`, ПРИНЯТО, T3): Go + chi (бэкенд), React + TypeScript (фронтенд). Открытые решения: БД, ORM; стилизация фронта: Tailwind CSS (принят в `ADR-DES.STACK.framework-vs-vs`). ADR уровня `DES` фиксируют инварианты, не зависящие от стека.
...
- **Стек — выбран (T3)**: язык (Go + TypeScript) и фреймворки (chi + React) приняты в `ADR-DES.STACK.*`; открыты БД/ORM и IMPL-детали (репозиторная структура). `DES`-уровень фиксирует инвариантные решения (границы сервиса, модель данных, API-контракт) независимо от оставшихся IMPL-выборов.
