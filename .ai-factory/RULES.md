...
- Stack is fixed (2026-08-02, ADR T3–T5): Go + chi + oapi-codegen backend (OpenAPI-first, modular monolith, single `vedo-edutrack` cobra binary), React + TypeScript SPA (Vite, Zustand, Tailwind CSS v4, React Flow + Cytoscape.js) frontend, PostgreSQL (sqlc + pgx, Atlas migrations), JWT RS256/JWKS (jwx), OTel → Prometheus/Loki/Tempo + Grafana, VEDO Hub as the external ontology platform; stack-independent invariants (boundaries, data model, API contract) stay at DES level.
